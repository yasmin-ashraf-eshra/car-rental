...
<body>
  <div class="sview">
    <h1 class="title">Car Reservation Report</h1>
    <nav>
      <ul>
    ...
    <form class="gsearch" action="car_reservation_report.php" method="post">
      <div class="container" style="height:115px">
        <label class="textlabel">Start Date</label>
        <input class="inputext" style="width:57%" type="date" name="start">
        <br><br>
        <label class="textlabel">End Date</label>
        <input class="inputext" style="width:58%" type="date" name="end">
        <br><br>
        <label class="textlabel">Car Plate Number</label>
        <input class="inputext" style="width:50%" type="text" name="car_id">
...
join reservation USING(plate_id)
join customer using (customer_id)
join office on office.office_id=reservation.office_id
WHERE reservation.pickup_date>='$start' and reservation.return_date<= '$end' and car.plate_id= '$car_id' ";

            $result = mysqli_query($conn, $query);
            if ($result->num_rows > 0) {
              // output data of each row
              while ($row = $result->fetch_assoc()) {
                echo "<tbody><tr><td scope='row'>" . $row['reservation_id'] . "</td><td>" . $row["fname"] . "</td><td>" . $row["Lname"] . "</td><td>" . $row["company"]
                  . "</td><td>" . $row["color"] . "</td><td>" . $row["model"] . "</td><td>" . $row["status"] . "</td><td>" . $row["year"] . "</td><td>" . $row["mileage"]
                  . "</td><td>" . $row["type"] . "</td><td>" . $row["insurance"] . "</td><td>" . $row["address"]
